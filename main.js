...
const db = {
    ...
    questions: [
      ...
      },
      {
        type: "filter",
        key: "petFriendly",
        question: "반려동물과 함께 입주할 수 있어야 하나요?",
        answers: [
          { text: "네, 필수예요", value: true },
          { text: "아니요, 괜찮아요", value: false },
        ],
      },
      {
          type: "filter",
    ...
    return `
      <div class="result-item">
        <a href="${facility.link}" target="_blank">
          ...
          <h4>${facility.name}</h4>
          <p>${facility.region} | ${facility.petFriendly ? '반려동물 가능' : '반려동물 불가능'}</p>
        </a>
      </div>
    `;
}
...
function displayAllFacilities() {
    ...
    recommendedFacilities.forEach(facility => {
